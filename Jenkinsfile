    ...
    stages {
        ...
        stage('Export & Upload JWA to Nexus (joget-repo)') {
            steps {
                ...
                sh '''
                    # Installer curl dans le conteneur jogetapp
                    docker exec -u root jogetapp bash -c "apt-get update && apt-get install -y curl"
                    ...
                    EXPORT_URL="http://localhost:8080/jw/web/json/apps/export?appId=${APP_ID}"
